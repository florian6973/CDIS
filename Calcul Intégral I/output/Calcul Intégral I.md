...
::: {.section}
A noter que [le théorème de changement de variable (p.
`\pageref*{changement-de-variable}`{=tex})](#changement-de-variable)
nous fournit également un moyen alternatif pour définir l'intégrale
entre $-\infty$ et $+\infty$, en nous remanant à une intégrale sur un
intervalle borné.
:::
...
$J_i \not \subset \gamma(t)$. Soit $t_i$ un point de $J_i$ ; la suite
des ces points appartient à $J_0$ qui est compact et admet donc une
suite extraite qui converge. Comme la suite des $t_k$ appartient à $J_i$
pour tout $k \geq i$, cette limite $t$ adhère à tous les $J_i$, et donc
...
nous avons ainsi bien défini une fonction de jauge sur $[a, b]$. Si
...
l'intégrale de Henstock-Kurzweil qu'il n'existe pas d'intégrale
...
$f: [a, b] \to \mathbb{R}$. La fonction $f$ est intégrable su sens de
...
$$ soit avec la [la convention des bornes inversées (p.
...
\end{array}
\right.
$$ Si la subdivisoin pointée $\mathcal{D}$ de $[a, b]$ est subordonnée à
...
cette fonction est continue, comme composée et produit de fonction
...
[^9]: Techniquement Jaroslav Kurzweil a inventé cette construction avant
    Ralph Henstock dans les années 1950, mais dans un but bien précis --
    l'étude des équations différentielles généralisées -- probablement
    sans réaliser totalement la portée de sa définition)
