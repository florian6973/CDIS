...
`\addcontentsline{toc}{subsubsection}{Proposition -- Propriétés élémentaires}`{=latex}
...
7.  Pour tout $x\in\mathbb{R}$,
    ...
    $t\to+\infty$. On en conclut que $h$ n'est pas (absolument)
    intégrable.

8.  La fonction de répartition $F$ considérée respecte bien les
    développements asymptotiques $(1)$ et $(2)$, mais la fonction $h$
    associée n'est pas (absolument) intégrable. Nous avons donc montré
    que la réciproque de la propriété est fausse.
:::
...
-   **Domaine.** $f : [-\infty,+\infty] \to \mathbb{R}^+$.
...
-   **Dérivabilité.** $f$ est continue sur $[-\infty,+\infty]$ et
    ...
    seconde $f^{\prime\prime}(x) =(x^2 - 1)\,f(x)$. Ces deux dernières
    fonctions peuvent être étendues à la droite réelle achevée en posant
...
2.  Par définition, pour tout $x\in\mathbb{R}$
    ...
    (c'est une densité ), son intégrale sur $[-\infty,0]$ (i.e. $F(0)$)
    vaut $\dfrac{1}{2}$. Lorsque $x > 0$, en décomposant l'intégrale
    puis en utilisant un petit changement de variable on obtient
...
3.  Soit $x>0$. Par définition,
    ...
    fondamental du calcul (extension) on a
...
4.  Soit $x>0$. Par définition,
    ...
    alors par le théorème fondamental du calcul (extension)
...
6.  Pour tout $x \in [-\infty,+\infty]$ on a
    ...
    conséquent, (d'après l'extension théorème fondamental du calcul )
    l'intégrale de $\left|h\right|$ sur $[0,+\infty]$ existe, et vaut
